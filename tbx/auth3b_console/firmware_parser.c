...
// -------------------- Class8 (Corrected Table Logic) --------------------

static uint8_t byte_0[256];
static uint8_t byte_1[256];
static uint8_t byte_2[256];
static int class8_inited = 0;

static void class8_smethod_0(void) {
    static const uint8_t raw_b1[256] = {
        ...
        0x38, 0xFE, 0x21, 0x58, 0x8C, 0xD8, 0xB8, 0x56, 0x7E, 0x2D, 0x6C, 0x82, 0xAE, 0xE2, 0x5E, 0x3B,
    ...
    memcpy(byte_1, raw_b1, 256);

    for (int i = 0; i < 256; i++) {
        byte_2[byte_1[i]] = (uint8_t)i;
    }
}

// Explicit copy of byte_0 table from VB smethod_1
static void class8_smethod_1(void) {
    static const uint8_t raw_b0[256] = {
...
// Corrected xform: read fw at 'fw_off', read graph at 'graph_base + fw_off'
static int class8_xform_block(const char* fw_path, const char* graph_path, int64_t fw_off, int len, int64_t graph_base, uint8_t* out) {
    ...
    for (int i = 0; i < len; i++) {
        out[i] = (uint8_t)(byte_0[a[i]] ^ byte_2[b[i]]);
    }
...
static int samsung_get_key(const char* path, char* key_out, size_t key_sz, int64_t* key_off_out, char* err, size_t errsz) {
    FILE* f = fopen(path, "rb");
    if (!f) { snprintf(err, errsz, "File open failed"); return -1; }

    int num = 0;
    uint8_t buf[21];

    while (1) {
        if (_fseeki64(f, 16384 + (int64_t)num * 21, SEEK_SET) != 0) { fclose(f); return -1; }
        if (fread(buf, 1, 21, f) != 21) { fclose(f); return -1; }

        if (buf[0] == 0xFF) break; // Found the padding

        num++;
        if (num > 1000) {
            fclose(f);
            snprintf(err, errsz, "SAMSUNG KEY SECTOR FAIL (scan limit)");
            return -1;
        }
    }

    if (num == 0) { // If B0 is FF, num is 0.
        fclose(f);
        snprintf(err, errsz, "SAMSUNG KEY SECTOR FAIL");
        return -1;
    }

    // Offset Calculation Correction:
    // In VB, 'num' increments after read, so final num = Index_of_FF + 1. Offset = (num - 2) = (Index_of_FF - 1).
    // In C, 'num' is exactly Index_of_FF. Offset = num - 1.
    int64_t final_off = 16384 + (int64_t)(num - 1) * 21 + 5;

    if (_fseeki64(f, final_off, SEEK_SET) != 0) { fclose(f); return -1; }
    uint8_t key[16];
    if (fread(key, 1, 16, f) != 16) { fclose(f); return -1; }

    char hex[33];
    bytes_to_hex_upper(key, 16, hex);
    strncpy(key_out, hex, key_sz);
    *key_off_out = final_off;
...
// Offsets for scattered key reconstruction (DG16D2S / DG16D4S)
static const int lo_scatter_offsets[16] = {
    0x138, 0xAEC, 0x742, 0x680,
    0xECD, 0xB34, 0x83F, 0x725,
    0x52A, 0x201, 0x9DF, 0x23,
    0x4A0, 0x59A, 0x3FE, 0xF49
};

static int liteon_get_key(const char* vendor, const char* model, const char* type, const char* path,
    char* key_out, size_t key_sz, int64_t* key_off_out, char* err, size_t errsz) {

    char mtrim[32], ttrim[32];
    trim_copy(mtrim, sizeof(mtrim), model);
    trim_copy(ttrim, sizeof(ttrim), type);

    if (strcmp(mtrim, "DG16D2S") == 0) {
        const char* graph = "graph.dll";
        if (!file_exists(graph)) { snprintf(err, errsz, "graph.dll missing"); return -1; }

        // Find Table Pointer using 0x14800 (83968) bank
        int64_t num4 = class8_find_hex_pattern(path, graph, 83968, "7C017D387F00", 4096, 0, -1);
        if (num4 == -1) { snprintf(err, errsz, "DG16D2S KEY TABLE NOT FOUND"); return -1; }

        // Read pointer address from (found - 12)
        uint8_t buf[8];
        if (!class8_xform_block(path, graph, num4 - 12, 8, 83968, buf)) {
            snprintf(err, errsz, "Read fail on pointer"); return -1;
        }

        // Calculate Base Address (VB: array4[0]<<16 + array4[2]<<8)
        int64_t base = ((int64_t)buf[0] << 16) + ((int64_t)buf[2] << 8);

        // Reconstruct scattered key
        char full_key[33] = { 0 };
        for (int i = 0; i < 16; i++) {
            uint8_t b;
            if (!class12_read_bytes(path, base + lo_scatter_offsets[i], 1, &b)) {
                snprintf(err, errsz, "Read fail scatter %d", i); return -1;
            }
            char h[3];
            bytes_to_hex_upper(&b, 1, h);
            strcat(full_key, h);
        }
        strncpy(key_out, full_key, key_sz);
        *key_off_out = base;
        return 0;
    }
    else if (strcmp(mtrim, "DG16D4S") == 0) {
        // FW 9504 / 0272 -> Scattered
        if (strcmp(ttrim, "9504") == 0 || strcmp(ttrim, "0272") == 0) {
            const char* graph = "graph.dll";
            if (!file_exists(graph)) { snprintf(err, errsz, "graph.dll missing"); return -1; }

            int64_t num6 = -1;
            int found = 0;
            int64_t search_start = 65536;

            // Loop using 0x54800 (346112) bank
            for (int i = 0; i < 50; i++) {
                num6 = class8_find_hex_pattern(path, graph, 346112, "7D387C01E4FF", 4096, search_start, -1);
                if (num6 == -1) break;

                uint8_t ver[2];
                // Check verification bytes (num6 - 8) using same bank
                if (class8_xform_block(path, graph, num6 - 8, 2, 346112, ver)) {
                    if (ver[0] == 84 && ver[1] == 15) {
                        found = 1;
                        break;
                    }
                }
            ...
            if (!found) { snprintf(err, errsz, "DG16D4S KEY TABLE NOT FOUND"); return -1; }

            // Pointer Read (num6 - 13 + 3) -> num6 - 10
            uint8_t buf[8];
            if (!class8_xform_block(path, graph, num6 - 10, 8, 346112, buf)) {
                snprintf(err, errsz, "Read fail on pointer"); return -1;
            }

            // Logic: num7 = array5(0) << 8; array5(5) = array5(5) >> 4; num7 += array5(5) << 16;
            int64_t base = ((int64_t)buf[0] << 8);
            uint8_t b5 = buf[5] >> 4;
            base += ((int64_t)b5 << 16);
        ...
        else if (strcmp(ttrim, "0225") == 0 || strcmp(ttrim, "0401") == 0 || strcmp(ttrim, "1071") == 0) {
            snprintf(err, errsz, "Encrypted Key (AES) - Extraction not implemented in this C port");
            return -1;
        }
        else {
            snprintf(err, errsz, "UNKNOWN LITEON DG16D4S ROM"); return -1;
        }
    }
    else if (strcmp(mtrim, "DG16D5S") == 0) {
        // Direct Read (VB smethod_11 uses offset 608256, but that's for parsing other data. 
        // Key is at 126960, and usually unencrypted in pre-processed dumps, or unreadable if locked.
        // Assuming user has a valid dump where key is at 126960)
        int64_t off = 126960;
        uint8_t buf[16];
        if (!class12_read_bytes(path, off, 16, buf)) {
            snprintf(err, errsz, "Read failed"); return -1;
        }
        char hex[33];
        bytes_to_hex_upper(buf, 16, hex);
        strncpy(key_out, hex, key_sz);
        *key_off_out = off;
        return 0;
    }

    snprintf(err, errsz, "UNKNOWN LITEON IDENTIFIER");
    return -1;
}
...
int cmd_parse_fw(const char* path, int json) {
    ...
    if (strcmp(vtrim, "HITACHI") == 0) {
        ...
        samsung_get_key(path, key, sizeof(key), &key_off, key_err, sizeof(key_err));
    }
    else if (strcmp(vtrim, "LITEON") == 0) {
        liteon_get_key(vendor, model, type, path, key, sizeof(key), &key_off, key_err, sizeof(key_err));
    }
    else if (strcmp(vtrim, "BENQ") == 0) {
